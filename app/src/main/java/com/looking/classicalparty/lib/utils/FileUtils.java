...
public class FileUtils {
    ...
    private static String getAppDir() {
        return Environment.getExternalStorageDirectory() + "/classical";
    }
    ...
    public static String getLrcFilePath(MusicBean music) {
        ...
        String lrcFilePath = getLrcDir() + getLrcFileName(music.getMusername(), music.getTitle());
        if (!exists(lrcFilePath)) {
            lrcFilePath = "http://www.jingdian.party/"+music.getV_path().replace(MP3, LRC);
            if (!exists(lrcFilePath)) {
                lrcFilePath = null;
            }
        }
     ...
     * @return 如果存在返回路径，否则返回null
     */
    public static String getAlbumFilePath(MusicBean music) {
        if (music == null) {
            return null;
        }

        String albumFilePath = music.getCover_path();
        if (TextUtils.isEmpty(albumFilePath) || !exists(albumFilePath)) {
            albumFilePath = getAlbumDir() + getAlbumFileName(music.getMusername(), music.getTitle());
